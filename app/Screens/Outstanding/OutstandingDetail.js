...
class OutstandingDetail extends Component {
    constructor(props) {
        super(props);
    ...
    getUnitDetail = () =>{
        ...
        {isMount ?
            ...
            fetch(urlApi+'c_myunits/myUnitOutstanding/'+entity_cd.trim()+'/'+project_no.trim()+'/'+debtor_acct,{
                method:'GET',
    ...
    // clickToPaid() {
    //     Actions.UnitPaymentSchedule();
    //     this.setState({ click: true });
    //   }
    handleOnlinePayment(data) {
        // coba yang ini bang
        // const getdataNup = this.state.nupss;
        Actions.OutstandingPayment({myunits : data, unit: this.props.unit})
        // this.setState({ click : true})
    }
                ...
                <Header style={Style.navigation}>
                    ...
                    <View style={Style.actionBarMiddle}>
                        <Text style={Style.actionBarText}>
                            {"Outstanding Detail".toUpperCase()}
                        </Text>
                    </View>
                            ...
                            <View style={{
                                ...
                                <View style={{flex:1}}>
                                    <Text style={[{
                                        ...
                                        Style.textBlue]}>Rp. {numFormat(data.trx_amt)}</Text>
                                </View>
                                ...
                                <View style={{flex: 0, alignItems:'flex-end', alignContent: 'center', alignSelf: 'center'}}>
                                    {
                                        data.status_paid == 'P' ?
                                        <TouchableOpacity 
                                            style={{ borderRadius: 10, backgroundColor: '#3CE543', width: 60, height: 30 }}
                                            onPress={() =>
                                                this.handleOnlinePayment(data)
                                            }
                                            // disabled={true}
                                            >
                                            <Text style={{
                                        ...
                                        :
                                            data.status_paid == 'O' ?
                                        <TouchableOpacity  
                                            style={{ borderRadius: 7, backgroundColor: '#FF9652', width: 60, height: 30 }}
                                            onPress={() =>
                                                this.handleOnlinePayment(data)
                                            }
                                        ...
                                        <TouchableOpacity  
                                            style={{ borderRadius: 7, backgroundColor: '#DE2828', width: 60, height: 30 }}
                                            // onPress={()=>Actions.UnitPaymentSchedule({
                                            //     unit: this.props.unit,
                                            //     duedate: data.due_date,
                                            //     Amount: data.trx_amt,
